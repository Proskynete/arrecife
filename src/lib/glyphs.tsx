/**
 * Glyphs are SVG, never emoji and never an icon package. These are the six the
 * primitives genuinely need; this is not an icon library and it does not grow
 * unless a new primitive asks for it.
 *
 * They all inherit `currentColor` and measure 1em, so color and size come from
 * the token of whatever context they are mounted in.
 */
...
/**
 * The one exception to «no movement»: a loading button that does not spin is
 * indistinguishable from a disabled one. It is feedback about progress, not
 * about state, and `motion-safe` turns it off for anyone who asked for less
 * motion.
 */
...
/**
 * The camera for the avatar upload control. It is here because a new primitive
 * asked for it, which is the only reason this file ever grows.
 */
export const Camera = (props: GlyphProps) => (
  <Glyph {...props}>
...
/**
 * The two for the theme control. Drawn at 16 like the rest, and the sun carries
 * its rays as a single `path` so it inherits `Glyph`'s stroke width with no
 * exceptions.
 */
...
/**
 * The player's eight glyphs, brought over verbatim from
 * `eduardoalvarez.dev/src/assets/icons`. The paths were not retouched: they are
 * the same ones already visible on the portfolio.
 *
 * They use viewBox 24 rather than 16 like the rest, because that is how they
 * were drawn and rescaling them by hand would have changed the stroke width.
 */
function MediaGlyph({ children, ...props }: GlyphProps) {
  return (
    <svg
...
export const Play = (props: GlyphProps) => (
  <MediaGlyph fill="currentColor" {...props}>
...
export const GoBackSeconds = (props: GlyphProps) => (
  <MediaGlyph stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" {...props}>
