# Manual de marca · Eduardo Álvarez · v1 · 2026

> **This file is a verbatim transcription and is deliberately NOT translated.**
>
> Extracted from `Manual de marca - Eduardo Alvarez.html` (a Claude Design
> canvas) on 27 Aug 2026. Same conditions as `design-system.md`: the canvas is
> the Spanish source and this copy only earns its place while it matches it word
> for word.

MANUAL DE MARCA · EDUARDO ÁLVAREZ · v1 · 2026 
eduardoalvarez.dev 
MANUAL DE MARCA · VERSIÓN 1 · 2026 
Eduardo Álvarez 
Identidad visual y normas de uso. Engineering leadership, plataforma y la era de la IA — con un tiburón ballena de por medio. 
01 · Concepto 
08 · Voz y tono 
02 · Arquitectura de marca 
09 · Aplicaciones por superficie 
03 · Sistema de marca 
10 · Charlas 
04 · Usos incorrectos 
11 · Merch y producción 
05 · Tiburoncín 
12 · Redes y OG 
06 · Color 
13 · Checklist 
07 · Tipografía 
— · Colofón 
01 · Concepto 
El tiburón ballena es el pez más grande del océano y es inofensivo. Filtra volúmenes enormes de agua por sus branquias para quedarse con lo mínimo que lo alimenta. Esa es la marca: profundidad sin intimidación, criterio para filtrar el ruido, y ganas de compartir lo que queda. 
Tres ideas sostienen el sistema. Si una decisión visual no apoya ninguna de las tres, sobra. 
FILTRAR 
Volumen, no ruido 
Dejar pasar el hype y quedarse con lo que alimenta al equipo. Es el trabajo de un líder técnico en la era de la IA. 
ACERCARSE 
Grande y accesible 
Los buzos nadan al lado del animal sin miedo. La seniority no se comunica poniendo distancia. 
RECONOCER 
Manchas como huella 
El patrón de manchas de cada tiburón ballena es único y se usa para identificarlo de por vida. Es identidad, no decoración. 
02 · Arquitectura de marca 
Hay tres nombres en circulación y cada uno tiene un puesto fijo. Confundirlos es el error más frecuente. 
NOMBRE 
ROL 
DÓNDE APARECE 
Eduardo Álvarez 
La firma. Es la marca. 
Logo, portadas, facturas, CV, contratos, propuestas. 
Proskynete 
El handle. Identidad en plataformas. 
GitHub, X, npm. Nunca en material impreso ni en el logo. 
Tiburoncín 
La mascota. El personaje. 
Ilustración, stickers, estados vacíos, cierre de charla. 
Regla: la mascota es el logo, pero el logo dice «Eduardo Álvarez». Tiburoncín nunca aparece escrito dentro del logotipo. 
03 · Sistema de marca 
Cuatro niveles, del más ilustrado al más mínimo. Una ilustración detallada no sobrevive a 16 px ni a un bordado, así que el sistema baja en detalle a medida que baja el tamaño. Elige el nivel por tamaño de reproducción, no por gusto. 
Eduardo Álvarez 
code · build · explore 
NIVEL 1 · LOCKUP COMPLETO 
Mascota en loto, wordmark en Bricolage Grotesque 700 uppercase con tracking 0.04em, y la línea code · build · explore en mono con tracking 0.28em. 
Portadas de charla, primera diapositiva, poleras, banner de stand. Mínimo 120 px o 30 mm de alto. 
NIVEL 2 · AVATAR 
La cabeza centrada dentro de un círculo azul tiburón #3E7CB1 sólido, sangrando ligeramente por el borde inferior. Fondo plano, nunca degradado. 
Foto de perfil en GitHub, LinkedIn, X, Instagram, Discord. Mínimo 36 px. 
NIVEL 3 · ISOTIPO (ALETA) 
Favicon, marca de agua en bloques de código, bordado, pin metálico, sello en la esquina de una diapositiva. Es la única pieza que aguanta 16 px y una sola tinta. 
Dos variantes obligatorias: la aleta a dos azules sobre fondo claro, y la silueta en espuma #EDF4F3 a una tinta sobre fondo oscuro. El cuerpo de la aleta es casi negro y desaparece sobre fondos oscuros. 
~/ eduardoalvarez.dev 
NIVEL 4 · FIRMA CLI 
La estética terminal con rol definido: es la firma tipográfica del sistema, no el fondo de todo. El prefijo ~/ en bioluz, el cursor de bloque en arena. 
Footers, navegación, transiciones de diapositiva, pie de firma en correo. 
Zona de seguridad y tamaños mínimos 
El aire libre alrededor del lockup equivale a la altura de la aleta dorsal de la mascota. Ningún elemento —texto, borde, foto, otro logo— entra en esa zona. En el avatar, la cabeza ocupa el 88 % del diámetro del círculo. 
PIEZA 
MÍN. PANTALLA 
MÍN. IMPRESO 
Lockup completo 120 px de alto 30 mm de alto 
Mascota sola 80 px de alto 20 mm de alto 
Avatar 36 px 12 mm 
Aleta 16 px 6 mm 
04 · Usos incorrectos 
La mascota es un activo ilustrado y frágil. Estas ocho cosas la degradan y no se hacen nunca, ni «solo esta vez». 
01 
Recolorear la mascota. El azul y el patrón de manchas SON la identificación del animal. Sin ellos no es un tiburón ballena. 
02 
Rotarla, inclinarla o deformarla para que quepa en un espacio. Se cambia el espacio o se cambia de pose. 
03 
Quitarle el contorno casco. Ese contorno es lo que la hace legible sobre cualquier fondo. 
04 
Ponerla sobre fotografía o sobre degradados de más de dos paradas. 
05 
Usarla como fondo con opacidad baja. No es una textura ni una marca de agua; para eso está la aleta. 
06 
Dos mascotas en la misma pieza. Compiten entre sí y ninguna gana. 
07 
La aleta a dos azules sobre fondo oscuro. Su cuerpo es casi negro: desaparece. Va la silueta en espuma. 
08 
Caras expresivas fuera de su sitio. Nunca en hero, precios, servicios, contacto ni CV. 
05 · Tiburoncín 
La mascota tiene nombre propio para que la gente la pida: «¿tienes el sticker del tiburón durmiendo?». Un personaje con nombre se colecciona; un dibujo genérico se ignora. 
Escritura 
Siempre con tilde y T mayúscula en texto corrido: Tiburoncín . Sin tilde y en minúscula en handles, dominios y nombres de archivo: tiburoncin . No se traduce ni se abrevia, en ningún idioma. 
El contrato de humor 
Las caras expresivas van solo en estados vacíos, confirmaciones, errores, progreso de curso y celebración. Nunca en hero, precios, servicios, contacto ni CV. Esta es la regla que mantiene la marca profesional: el humor vive en los momentos donde el usuario ya no está evaluando si contratarte. 
Inventario de caras 
ARCHIVO 
EXPRESIÓN 
USO ASIGNADO 
head-1 Guiño sonriente Formularios, nota al pie amable 
head-2 Ojos cerrados, sonrisa Sin resultados, estado de espera 
head-3 Risa grande Éxito, suscripción confirmada 
head-4 Lentes de sol Módulo o curso completado 
head-5 Sorpresa Confirmación destructiva, aviso 
head-6 Ojos de corazón Agradecimiento, OG de artículo 
head-7 Confusión 404, página no encontrada 
head-8 Molesto Error del servidor, fallo de build 
Inventario de poses 
laptop-coffee 
sleeping 
snorkel 
books 
headphones 
surf 
jump-logos 
desk 
Además: hug-laptop , peek y backpack . Las poses nuevas se encargan con este manual en la mano: mismo grosor de línea, mismo patrón de manchas, mismo contorno casco. 
06 · Color 
La paleta se deriva del animal y del océano. Los neutros llevan azul real, no gris: es lo que integra la mascota con la interfaz en vez de dejarla pegada encima. El acento primario es bioluminiscencia, no el cian de stock; el secundario es un tono cálido de arena, y es lo que evita que el sistema se sienta rígido. 
Modo oscuro · primario 
NOMBRE 
HEX 
USO 
CONTRASTE 
abismo #091319 Fondo de página — 
fosa #10202B Tarjetas y paneles — 
corriente #17303E Elevado, menús, tabs activos — 
borde #22414F Bordes de control — 
espuma #EDF4F3 Titulares y cuerpo 16.9:1 
bruma #A7BCC4 Cuerpo secundario 8.9:1 
plancton #71919C Metadatos · solo ≥13px 4.9:1 
bioluz #35D6C0 Interactivo: links, foco, terminal 10.7:1 
arena #F2A65A Humano: categorías, citas, conversión 9.4:1 
tiburón #3E7CB1 Cuerpo de la mascota · solo relleno 4.3:1 
manchas #C2D7E7 Patrón de manchas — 
casco #0B1524 Contorno + bloques de código — 
Modo claro · lectura larga, PDF, impresión 
NOMBRE 
HEX 
USO 
papel #F6F2EA Fondo. Blanco cálido obligatorio 
papel alto #FFFFFF Tarjetas sobre papel 
bioluz oscura #0F8F80 Links y foco sobre papel · 4.6:1 
arena oscura #B4632A Etiquetas y citas sobre papel · 4.8:1 
Nunca blanco puro como fondo de página. Sobre #FFFFFF el contorno casco vibra y la panza del tiburón desaparece. 
En claro el botón primario no puede ser bioluz ni arena —no dan contraste con texto oscuro encima—: pasa a casco sólido, y los acentos viven en sus variantes oscuras para texto y bordes. 
Proporción y reparto 
60 % abismo · 30 % fosa y superficies · 7 % azul tiburón · 3 % acentos. Si en una pieza hay más acento que eso, el acento dejó de acentuar. Máximo dos paradas en cualquier degradado. 
Bioluz es lo interactivo —enlaces, foco, estado activo, terminal—. Arena es lo humano —etiquetas de categoría, citas, CTA de charlas y cursos, merch—. No se intercambian. 
07 · Tipografía 
Tres familias con roles que no se pisan: una con carácter para titular, una neutra para leer, una monoespaciada para la máquina. Un grotesco neutro en titulares es lo que hacía que la marca anterior se sintiera rígida. 
Bricolage Grotesque 
Titulares y números grandes. Pesos 600 / 700 / 800, tracking −0.03em. Terminaciones ligeramente irregulares: da calidez y humor sin caer en tipografía redonda infantil. Nunca en cuerpo de texto. 
Geist · Aa Bb Cc 0123 
Cuerpo e interfaz. Pesos 300 / 400 / 500. Se mantiene del sistema anterior porque es excelente para leer. Cuerpo de artículo 18 px / 1.75, interfaz 15 px, etiquetas 13 px con peso 500. 
JetBrains Mono · ~/ 0123 
Código, rutas, etiquetas de sección, metadatos y la firma CLI. Reemplaza a Geist Mono: tiene ligaduras y un cero ranurado más legible en proyección. 
ROL 
FAMILIA 
TAMAÑO 
PESO 
TRACKING 
display Bricolage 76px / 0.96 800 −0.035em 
h1 Bricolage 44px / 1.05 700 −0.03em 
h2 Bricolage 30px / 1.1 600 −0.02em 
h3 Bricolage 25px / 1.15 600 −0.02em 
cuerpo Geist 18px / 1.75 400 0 
interfaz Geist 15px / 1.6 400 / 500 0 
etiqueta Geist 13px / 1.5 500 0 
eyebrow JetBrains 12px 500 0.10–0.14em 
Mínimos absolutos: 13 px en pantalla, 12 pt impreso, 24 px en una diapositiva de 1920×1080. Medida máxima de cuerpo: 68 caracteres. 
08 · Voz y tono 
Diez años de industria hablando en primera persona, sin distancia y sin pose. La autoridad viene de haber estado ahí, no del vocabulario. 
SÍ 
Contar cicatrices propias con nombre y fecha. 
Decir «no lo sé» y «esto me salió mal». 
Frases cortas. Un dato concreto por párrafo. 
Humor seco, y solo donde el contrato lo permite. 
Segunda persona cuando enseñas: «te va a pasar que…». 
NO 
Vender certezas sobre la IA que nadie tiene. 
Jerga por jerga: «sinergia», «disruptivo», «10x». 
Listas de tres adjetivos donde va un ejemplo. 
Emoji en material de marca. 
Hablar de «nosotros» corporativo. Eres una persona. 
Los títulos de sección usan el formato de ruta — ./artículos , ./hablemos — y los enlaces de acción el formato ./leer → . Es la parte de la estética terminal que sobrevive en el texto. 
09 · Aplicaciones por superficie 
El mismo sistema se dosifica distinto según lo que la persona esté haciendo ahí. El portfolio es sobrio, los cursos son cálidos, los links son la pieza más personal. 
SUPERFICIE 
MODO 
ROL DE LA MASCOTA 
ACENTO 
eduardoalvarez.dev Oscuro Firma: aleta en nav y footer, una pose en el hero, caras solo en 404 y newsletter bioluz 
links Oscuro Protagonista: pose grande arriba arena en el CTA 
cursos Claro Acompaña: una pose por ruta, caras en progreso y celebración arena 
Admin del blog Oscuro Solo la aleta. Sin caras salvo estados vacíos bioluz 
Slides Oscuro Portada y cierre; en el medio solo la aleta en la esquina bioluz 
Merch Ambos Manda la mascota; el nombre casi desaparece ambos 
10 · Charlas 
Tres plantillas, y nada más. Lienzo 1920×1080, texto nunca bajo 24 px, cuerpo real entre 30 y 44 px. 
CARIBECONF 2026 
Microfrontends
sin dolor 
01 · portada 
PARTE 02 
Cuándo NO usarlos 
02 · sección 
Tres señales de alerta 
01 Un solo equipo mantiene el frontend 
02 El despliegue ya es rápido 
03 No existe design system compartido 
03 · contenido 
La diapositiva de sección en arena pleno aguanta dos o tres líneas de titular, no más. Se usa cada tres o cuatro slides para marcar el ritmo. 
11 · Merch y producción 
El merch es donde la mascota manda y el nombre casi desaparece. Nadie pega un sticker por el nombre; lo pega porque el dibujo le gusta. La URL va chica y siempre presente. 
PIEZA 
ESPECIFICACIÓN 
ARTE 
Sticker die-cut 55 mm · vinilo mate · contorno blanco 2 mm Una pose + URL al pie 
Sticker de cara 25 mm · vinilo mate Una cara, sin texto 
Polera negra Print 90 mm al pecho · dos tintas Aleta espuma + «FILTRA EL RUIDO» + URL 
Polera arena Tela #F6F2EA · print 140 mm Mascota grande + BUILD · LEARN · SHARE. URL en la manga 
Poleron Bordado 40 mm al pecho · una tinta Solo la aleta 
Pin metálico 25 mm · esmalte duro · dos colores Solo la aleta 
Regla de producción: todo el merch debe funcionar en dos tintas sobre tela oscura y sobre tela clara. Antes de mandar a imprimir, convierte la pieza a escala de grises: si la mascota se pierde contra el fondo, el contorno casco no es suficiente y hay que añadir contorno espuma de 2 mm. 
12 · Redes y OG 
Las imágenes de redes se generan en el build, no se diseñan a mano. Dos plantillas a 1200×630. 
ENGINEERING-CULTURE · 8 MIN 
El camino hacia mi primera charla internacional 
eduardoalvarez.dev 
og · artículo 
NUEVO CURSO 
Microfrontends sin dolor 
6 módulos · 4h 20m 
og · curso 
Avatar de perfil: nivel 2 en todas las plataformas, idéntico, sin variantes por red. La coherencia del avatar es lo que hace que te reconozcan en el timeline. 
13 · Checklist 
Antes de publicar una pieza digital o mandar una a imprenta. 
DIGITAL 
☐ La aleta usa la variante correcta para el fondo. 
☐ Hay una sola mascota en la pieza. 
☐ Las caras están solo donde el contrato lo permite. 
☐ Un único botón arena por pantalla. 
☐ Ningún texto bajo 13 px. 
☐ El acento no pasa del 3 % de la superficie. 
☐ Los enlaces tienen color definido y estado hover. 
IMPRESO 
☐ Prueba en escala de grises: la mascota se separa del fondo. 
☐ El arte es vectorial, no PNG. 
☐ Se respetan los tamaños mínimos de la tabla 03. 
☐ Zona de seguridad libre alrededor del lockup. 
☐ Fondo en blanco cálido, nunca blanco puro. 
☐ Ningún texto bajo 12 pt. 
☐ La URL está presente en la pieza. 
COLOFÓN 
Manual de marca versión 1, agosto de 2026. Los assets de la mascota son mapas de bits: la siguiente versión de este manual llega cuando estén vectorizados. Hasta entonces, no se usan en bordado, pin ni print sobre 200 mm. 
~/eduardoalvarez.dev