...
const meta = { title: "Primitives/Badge", component: Badge } satisfies Meta<
  typeof Badge
>;
export default meta;
    ...
    <>
      <Block title="category · mono pill in sand">
        ...
        <Note>
          ...
          filter in the article listing, which is why it is the `active` prop
          and not a `className` at the call site.
        </Note>
      </Block>
      ...
      <Block title="status · square r6, sans 12.5/500, background at 8 %">
        ...
        <Note>
          No border. It had one for a while and it was heavy: a bordered box
          next to a title reads as a control and not as a datum. The text uses
          `textPrimary` and not the tone's color — switch the mode in the
          toolbar: in light the semantics are calibrated to pass JUST over
          paper, so over their own tint they fall to 4.10–4.40 and fail AA.
        </Note>
        <Note>
      ...
      <Block title="metric · mono, no box, no transform">
        ...
        <Note>
          ...
          badges': it turned a slug into `ENGINEERING-CULTURE` and a file name
