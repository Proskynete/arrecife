...
| `@eduardoalvarez/arrecife/icons` | yes | `@phosphor-icons/react` | `Icon`, which draws a Phosphor icon at the system's size and weight |
...
// 1em and weight `regular`, which is the stroke the identity document names
...
number. **Do not change the weight either.** Phosphor bakes the weight into the
path, and `regular` is 16 on a 256 grid = 0.0625em, against the document's 1.6 on
a 24 grid = 0.0667em. Six per cent apart, which is no pixel on any screen.
